...
namespace Solutions.Now.Moe.Elsa.Activities
{
    [Activity(
          Category = "Approval",
          DisplayName = "SiteVisitApprovalConsultant",
          Description = "Add approval in ApprovalHistory Table",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class SiteVisitApprovalConsultant : Activity
    {
        private readonly DesignReviewDBContext _DesignReviewDBContext;
        private readonly SsoDBContext _ssoDBContext;
        private readonly MoeDBContext _moeDBContext;

        public SiteVisitApprovalConsultant(DesignReviewDBContext DesignReviewDBContext, SsoDBContext ssoDBContext, MoeDBContext moeDBContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
           ...
           try
            { 
                 List<WorkFlowRules> workFlowRules = _moeDBContext.WorkFlowRules.AsQueryable().Where(s => s.workflow == WorkFlowsName.sitevisitConsultant && s.type == WorkFlowType.WorkflowType).OrderBy(s => s.step).ToList<WorkFlowRules>();
                ...
                for (int i = 0; i < workFlowRules.Count; i++)
                {
                    ...
                    ProjectsTender projectsTender = await _moeDBContext.ProjectsTender.FirstOrDefaultAsync(s => s.tenderSerial == siteVisit.tenderSerial);
                    ReferedTender referedTender = await _moeDBContext.ReferedTender.FirstOrDefaultAsync(r => r.Serial == projectsTender.tenderSerial);
