...
namespace Solutions.Now.Moe.Elsa.Activities.Construction
{
    [Activity(
      ...
      DisplayName = "Work Schedule Approval",
      Description = "Work Schedule Approval in WorkflowRules Table",
      Outcomes = new[] { OutcomeNames.Done }
  )]
    public class Construction_WorkSchedule : Activity
    {
        private readonly ConstructionDBContext _ConstructionDBContext;
        private readonly SsoDBContext _ssoDBContext;
        private readonly MoeDBContext _moeDBContext;
        public Construction_WorkSchedule(ConstructionDBContext DesignReviewDBContext, SsoDBContext ssoDBContext, MoeDBContext moeDBContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRulesConstruction> workFlowRules = _ConstructionDBContext.WorkFlowRules.AsQueryable().Where(s => s.workflow == WorkFlowsName.Construction_WorkSchedule).OrderBy(s => s.step).ToList<WorkFlowRulesConstruction>();
            TblUsers users;
            ...
            try
            {
                var tender = await _ConstructionDBContext.Tender.FirstOrDefaultAsync(x => x.tenderSerial == 17);
                // المقاول
