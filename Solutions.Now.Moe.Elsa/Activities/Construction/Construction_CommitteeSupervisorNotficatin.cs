...
namespace Solutions.Now.Moe.Elsa.Activities.Construction
{
    [Activity(
       ...
       DisplayName = "Communication Supervisor Approval",
       Description = "Communication CommitteememberSupervisor Approval in WorkflowRules Table",
       Outcomes = new[] { OutcomeNames.Done }
   )]
    public class Construction_CommitteeSupervisorNotficatin : Activity
    {
        private readonly ConstructionDBContext _ConstructionDBContext;
        private readonly SsoDBContext _ssoDBContext;
        private readonly MoeDBContext _moeDBContext;

        public Construction_CommitteeSupervisorNotficatin(ConstructionDBContext DesignReviewDBContext, SsoDBContext ssoDBContext, MoeDBContext moeDBContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            try
            {
                if (workflowType == WorkFlowsName.Construction_InitialReceipt)
                {
                    var initialreceipt = await _ConstructionDBContext.TakeOver.FirstOrDefaultAsync(x => x.serial == RequestSerial);

                    var committeeMembers = await _ConstructionDBContext.CommitteeMember.AsAsyncEnumerable().Where(c => c.type == 4812 &&c.tenderSerial== initialreceipt.tenderSerial).ToListAsync();
                ...
                else if (workflowType == WorkFlowsName.Construction_partial_Receipt_Works) 
                {
                    var partialTakeOvers = await _ConstructionDBContext.partialTakeOver.FirstOrDefaultAsync(x => x.serial == RequestSerial);

                    var committeeMembers = await _ConstructionDBContext.CommitteeMember.AsAsyncEnumerable().Where(c => c.type == 4812 && c.tenderSerial == partialTakeOvers.tenderSerial).ToListAsync();
                    for (int i = 0; i < committeeMembers.Count; i++)
                    {
                        if (!String.IsNullOrEmpty(committeeMembers[i].userName.ToString()))
                        {
