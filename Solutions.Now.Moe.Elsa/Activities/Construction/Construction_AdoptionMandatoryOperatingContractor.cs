...
namespace Solutions.Now.Moe.Elsa.Activities
{
    [Activity(
       ...
       DisplayName = "Construction_AdoptionMandatoryOperatingContractor Approval",
       Description = "Construction_AdoptionMandatoryOperatingContractor in WorkflowRules Table",
       Outcomes = new[] { OutcomeNames.Done }
   )]
    public class Construction_AdoptionMandatoryOperatingContractor : Activity
    {
        private readonly ConstructionDBContext _ConstructionDBContext;
        private readonly SsoDBContext _ssoDBContext;
        private readonly MoeDBContext _moeDBContext;

        public Construction_AdoptionMandatoryOperatingContractor(ConstructionDBContext DesignReviewDBContext, SsoDBContext ssoDBContext, MoeDBContext moeDBContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRulesConstruction> workFlowRules = _ConstructionDBContext.WorkFlowRules.AsQueryable().Where(s => s.workflow == WorkFlowsName.Construction_AdoptionMandatoryOperatingContractor).OrderBy(s => s.step).ToList<WorkFlowRulesConstruction>();
            TblUsers users;
            ...
            try
            {
                ...
                users = await _ssoDBContext.TblUsers.FirstOrDefaultAsync(u => u.Administration == tender.tenderSupervisor && u.position == Positions.DirectorateHead && (u.organization == 2 || u.organization == 3)); if (users != null)
                {
