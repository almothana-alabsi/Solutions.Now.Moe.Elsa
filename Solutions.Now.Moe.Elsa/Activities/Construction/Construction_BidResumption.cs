...
namespace Solutions.Now.Moe.Elsa.Activities.Construction
{
    [Activity(
           ...
           DisplayName = "Construction Bid Resumption approval",
           Description = "Construction Bid Resumption in ApprovalHistory Table",
           Outcomes = new[] { OutcomeNames.Done }
       )]
    public class Construction_BidResumption : Activity
    {
        private readonly ConstructionDBContext _ConstructionDBContext;
        private readonly SsoDBContext _ssoDBContext;
        private readonly MoeDBContext _moeDBContext;

        public Construction_BidResumption(ConstructionDBContext ConstructionDBContext, SsoDBContext ssoDBContext, MoeDBContext moeDBContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRulesConstruction> workFlowRules = _ConstructionDBContext.WorkFlowRules.AsQueryable().Where(s => s.workflow == WorkFlowsName.Construction_BidResumption).OrderBy(s => s.step).ToList<WorkFlowRulesConstruction>();
            TblUsers users;
            ...
            try
            {
                var tenderProjectResume = await _ConstructionDBContext.tenderProjectResume.FirstOrDefaultAsync(x => x.serial == RequestSerial);
                ...
                //رئيس اللجنة
                var committeeCaptain = await _ConstructionDBContext.CommitteeMember.FirstOrDefaultAsync(x => x.tenderSerial == tenderProjectResume.tenderSerial && x.type == WorkFlowsName.Construction_SupervisionCommittee && x.captain == 1);
