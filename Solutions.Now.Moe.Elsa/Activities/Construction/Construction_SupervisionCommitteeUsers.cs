...
namespace Solutions.Now.Moe.Elsa.Activities.Construction
{
    [Activity(
          ...
          DisplayName = "Construction Supervision Committee users approval",
          Description = "Construction Supervision Committee users in ApprovalHistory Table",
          Outcomes = new[] { OutcomeNames.Done }
      )]
    public class Construction_SupervisionCommitteeUsers : Activity
    {
        private readonly ConstructionDBContext _ConstructionDBContext;
        private readonly SsoDBContext _ssoDBContext;
        private readonly MoeDBContext _moeDBContext;

        public Construction_SupervisionCommitteeUsers(ConstructionDBContext ConstructionDBContext, SsoDBContext ssoDBContext, MoeDBContext moeDBContext)
        {
        ...
        protected override async ValueTask<IActivityExecutionResult> OnExecuteAsync(ActivityExecutionContext context)
        {
            ...
            List<WorkFlowRulesConstruction> workFlowRules = _ConstructionDBContext.WorkFlowRules.AsQueryable().Where(s => s.workflow == WorkFlowsName.Construction_SupervisionCommittee).OrderBy(s => s.step).ToList<WorkFlowRulesConstruction>();
            TblUsers users;
            ...
            try
            {
                var supervisionCommittee = await _ConstructionDBContext.SupervisionCommittee.FirstOrDefaultAsync(x => x.serial == RequestSerial);
                var tender = await _ConstructionDBContext.Tender.FirstOrDefaultAsync(x => x.tenderSerial == supervisionCommittee.tenderSerial);
                //رئيس قسم الابنية
